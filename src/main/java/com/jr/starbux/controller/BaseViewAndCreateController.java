...
@Log4j2
public abstract class BaseViewAndCreateController<T extends BaseModel, I, S extends BaseService > {
	...
	public T find(@PathVariable("id") I id) {
		...
		try {
			...
			if(entity == null) throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Id not found");
			return entity;
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, e.getMessage());
